...
function Icon({ width = '20px', height = '20px', color = 'orange', style = {} } = {}) {
  return (
    <svg
...
const Rate = (RateProps: RateProps) => {
  const {
    value: inputValue = 0,
    count = 5,
    allowHalf = false,
  ...
  useEffect(() => {
    if (!allowHalf) {
      setLastValue(parseInt(lastValue.toString(), 10));
    }
  }, [allowHalf]);

  const onStarMouseMove = (e: any, index: number) => {
    const v = e.clientX - e.currentTarget.offsetLeft > 10 || !allowHalf ? 1 : 0.5;
  ...
  return (
    <div className="df" style={{ marginLeft: 66 }} onMouseLeave={onStarBoxMouseLeave}>
      {// @ts-ignore
